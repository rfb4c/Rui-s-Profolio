...
const handleSubmit = async () => {
  try {
    ...
    const submitData = {
      ...eventForm.value,
      // 根据不同的图片输入类型设置eventImage
      eventImage: imageInputType.value === 'url' ? eventForm.value.imageUrl : previewUrl.value,
    }
    ...
    if (response.flag) {
      ElMessage.success(response.message || '事件创建成功')
      // 创建成功后返回事件列表页面
      router.push('/eventList')
    } else {
      ElMessage.error(response.message || '事件创建失败')
    }
  } catch (error) {
    console.error('Create event error:', error)
    ElMessage.error('事件创建失败：' + (error.message || '未知错误'))
  }
}
